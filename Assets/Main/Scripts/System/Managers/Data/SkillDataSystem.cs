...
public class SkillDataSystem
{
    ...
    private Dictionary<SkillID, Dictionary<int, GameObject>> levelPrefabDatabase = new();
    #endregion
    ...
    private void LoadAllSkillData()
    {
        try
        {
            ...
            Debug.Log($"[SkillDataSystem] Total loaded skill data count : {skillDatabase.Count}");
        }
        catch (Exception e)
        {
            Debug.LogError($"[SkillDataSystem] Error in LoadAllSkillData: {e.Message}");
        }
    }
