...
public class ElementalAmplifierEffect : SkillIneractionEffect
{
    private readonly List<ElementType> applicableElements;
    private readonly float elementalPowerBonus;

    public ElementalAmplifierEffect(ItemEffectData effectData)
        : base(effectData)
    {
    ...
    public override void ModifySkillStats(Skill skill)
    {
        ...
        if (!applicableElements.Contains(skillData.Element))
            return;

        var stats = skillData.GetSkillStats();
