...
public class HomingActivatorEffect : SkillIneractionEffect
{
    private readonly float homingRange;
    private readonly List<SkillType> applicableSkillTypes;

    public HomingActivatorEffect(ItemEffectData effectData)
        : base(effectData)
    {
    ...
    {
        if (!applicableSkillTypes.Contains(skill.skillData.Type))
            return;
        ...
        if (skillData != null)
        {
            var stats = skillData.GetSkillStats() as ProjectileSkillStat;
            if (stats != null)
            {
                stats.isHoming = true;
